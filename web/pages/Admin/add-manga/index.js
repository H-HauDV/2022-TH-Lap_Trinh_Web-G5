...
function AddManga() {
  ...
  const fetchTotalManga = async () => {
    ...
    axios.get("http://127.0.0.1:8000/api/manga/tags").then((response) => {
      setTags(response.data);
      //console.log(tags)
    });
  };
  const initCategoryOption = tags.map((tag) => (
    <Option key={tag.id}>{tag.category_name}</Option>
  ));
  useEffect(() => {
    fetchTotalManga();
  }, []);
  ...
  function checkMangaName(e) {
    var checkLink =
      "http://127.0.0.1:8000/api/checkManga/name/" + e.target.value;
          ...
          <Col className="" span={24}>
            ...
            >
              <Form.Item
                ...
                rules={[
                  { required: true, message: "Please input Manga name!" },
                ]}
              >
              ...
              <Form.Item
                ...
                name="mainImage"
                rules={[
                  { required: true, message: "Please input your Main image!" },
                ]}
              >
                <Upload {...props}>

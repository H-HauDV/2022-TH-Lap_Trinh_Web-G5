...
import { List, Card, Button, Tooltip, Table, Row, Col, Rate, Tag, Divider, Avatar } from 'antd';
...
function MangaPage() {
  ...
  const openChapterPage =(chapterName) =>{
    ...
    chapter.map(aChapter => {
      if(aChapter.chapter_name==chapterName) link= "/Public/chapter?id=" + aChapter.chapter_id;
    })
    router.push(link)
  }
    ...
    <div id="MainContent">
      ...
      <Row className="">
        <Col className="" span={15}>
          <Row style={{  backgroundColor: "#343a40", borderRadius: 5, marginBottom:20, padding: 20,}}>
            <Col style={{padding: 10}} className="" span={8}>
              <img alt="example" style={{ width: 240 }} src={manga.main_image} />
              <h3 className="text-white">Rating</h3>
              <Rate disabled defaultValue={2} />
            </Col>
            <Col className="" span={14} offset={2}>
              <h2 className="text-white" text-white>{manga.name}</h2>
              <div className="same-row text-white" style={{ marginBottom: 10, }}>
                <FontAwesomeIcon icon={faClone} />&nbsp; 
                <h4 className="text-white"><b> Tên gọi khác: &nbsp; </b></h4> 
                <h4><Tag color="#108ee9">{manga.alter_name}</Tag></h4>
              </div> 
              <div className="same-row text-white" style={{ marginBottom: 10, }}>
                <FontAwesomeIcon icon={faUsers} />&nbsp; 
                <h4 className="text-white"><b> Tác giả: &nbsp; </b></h4> 
                <h4><Tag color="#108ee9">{manga.author}</Tag></h4>
              </div> 
              <div className="same-row text-white" style={{ marginBottom: 10, }}>
                <FontAwesomeIcon icon={faTags} />&nbsp; 
                <h4 className="text-white"><b> Thể loại: &nbsp; </b></h4> 
                <h4 className="same-row "> 
                  {tags.map((tag) => (
                    <Tag style={{ height: 25, paddingLeft: 1, paddingRight: 1}}color="#f50">{tag.category_name}</Tag>
                  ))}
                </h4>
              </div> 
              <div className="same-row text-white" style={{ marginBottom: 10, }}>
                <FontAwesomeIcon icon={faSpinner} />&nbsp; 
                <h4 className="text-white"><b> Tình trạng: &nbsp; </b></h4> 
                <h4 className="text-white">{(manga.status=1) ? "Đang tiến hành": "Đã hoàn thành"} </h4>
              </div> 
              <div className="same-row  text-white" style={{ marginBottom: 10, }}>
                <FontAwesomeIcon icon={faUserPlus} />&nbsp; 
                <h4 className="text-white"><b> Nhóm dịch: &nbsp; </b></h4> 
                <h4 className="text-white">{manga.source} </h4>
              </div> 
              <div className="same-row text-white" style={{ marginBottom: 10, }}>
                <FontAwesomeIcon icon={faEye} />&nbsp; 
                <h4 className="text-white"><b> Lượt xem: &nbsp; </b></h4> 
                <h4 className="text-white">{viewCount} </h4>
              </div> 
              <div style={{ marginBottom: 15, }}>
                <Tooltip title="Follow">
                  <Button type="primary" style={{ backgroundColor:"#3f6791", borderColor:"#3f6791" }} >
              ...
              <div >
                <Tooltip title="Latest">
                  <Button type="primary" style={{ backgroundColor:"#e12e1c", borderColor:"#e12e1c" }}>
          ...
          <Card className="card-head" title="Danh sach chuong" bordered={false} style={{  backgroundColor: "#343a40", borderRadius: 5,}}>
            <Row style={{  }}>
              <Table scroll={{ y:400 }} 
                pagination={false} 
                dataSource={chapter} 

...
import { Layout, Menu, AutoComplete, Button, Avatar } from "antd";
const { Header } = Layout;
function Navbar() {
  ...
  const fetchBasicUserData = async () => {
    try {
      ...
      var apiLinkForBasicUserInfo =
        "http://127.0.0.1:8000/api/user/getInfoByID/advanced/" +
        userFromLocal.id;
      axios.get(apiLinkForBasicUserInfo).then((response) => {
  ...
  const onSearchInput = (searchText) => {
    ...
    if (searchText != null) {
      var apiLinkForSearchSuggestion =
        "http://127.0.0.1:8000/api/manga/search/suggestion/fromName/" +
        searchText;
      console.log(apiLinkForSearchSuggestion);
      axios.get(apiLinkForSearchSuggestion).then((response) => {
        // console.log(response.data)
        // setSearchOption()
  ...
  const onSearchSelect = (data) => {
    var mangaID = 0;
    var apiLinkForGetMangaID =
      "http://127.0.0.1:8000/api/manga/get/mangaID/fromName/" + data;
    axios.get(apiLinkForGetMangaID).then((response) => {
